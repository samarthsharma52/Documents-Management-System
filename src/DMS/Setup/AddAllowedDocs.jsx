...
import AllowedTable from "../components/AllowedTable";
import MessageModal from "../components/MessageModal";
import { fetchAllowedDocs, addAllowedDoc, deleteAllowedDoc } from "../service/Setup";

const AddAllowedDocs = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await addAllowedDoc(formData, token);
      setFormData({ documentName: "", description: "" });
      setResponseMessage("Allowed Document added successfully!");
      handleFetchDocuments();
      setSuccess(true);
      setConfirm(false);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await deleteAllowedDoc(id, token);
      handleFetchDocuments();
    } catch (error) {
      setResponseMessage(error.message);
      setSuccess(false);
